...
const MetricsShowcase = () => {
  ...
  { label: "CALMAR RATIO", value: "2.72", color: "text-purple-400" },
  ];
  ...
  <section className="w-full max-w-7xl mx-auto px-6 my-24">
      ...
      <button
        ...
        className="w-full flex justify-between items-center bg-white border border-gray-200 dark:bg-slate-900 dark:border-slate-700 px-6 py-4 rounded-2xl shadow-lg backdrop-blur-none hover:bg-gray-50 dark:hover:bg-slate-800 transition"
      >
        ...
        >
          <ChevronDown className="text-gray-700 dark:text-white w-6 h-6" />
        </motion.div>
      </button>
              ...
              <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-8">
                {highlights.map((item, idx) => (
                  ...
                  >
                    ...
                    <p className="text-gray-600 dark:text-slate-400 text-sm mt-2">{item.desc}</p>
                  </div>
                ))}
              </div>
            </div>
